...
function Login() {
  ...
  const dashboard = (e: any) => {
    ...
    if (
      values.name?.trim() === "moderator" &&
      values.password?.trim() === "12345678"
    ) {
      navigate.push("/dashboard");
      Toast({ title: "Welcome Moderator", error: false });
      const user = {
        name: "moderator",
      };
      dispatch(login(user));
    ...
    if (
      values.name?.trim() === "comms" &&
      values.password?.trim() === "12345678"
    ) {
      navigate.push("/dashboard");

      Toast({ title: "Welcome Comms", error: false });
      const user = {
        name: "comms",
      };
      dispatch(login(user));
    ...
    if (
      values.name?.trim() === "patriot" &&
      values.password?.trim() === "12345678"
    ) {
      navigate.push("/dashboard");

      Toast({ title: "Welcome Patriot", error: false });
      const user = {
        name: "patriot",
      };
      dispatch(login(user));
    ...
    if (
      values.name?.trim() === "admin" &&
      values.password?.trim() === "12345678"
    ) {
      navigate.push("/dashboard");

      Toast({ title: "Welcome Admin", error: false });
      const user = {
        name: "admin",
      };
      dispatch(login(user));
    ...
    if (
      values.name?.trim() === "superadmin" &&
      values.password?.trim() === "12345678"
    ) {
      navigate.push("/dashboard");

      Toast({ title: "Welcome Superadmin", error: false });
      const user = {
        name: "superadmin",
      };
      dispatch(login(user));
    ...
    if (values.name === "" || values.password === "") {
      Toast({ title: "Enter your credentials", error: true });
      return;
    } else {
      Toast({ title: "Invalid credentials", error: true });
    }
  };
  useEffect(() => {
